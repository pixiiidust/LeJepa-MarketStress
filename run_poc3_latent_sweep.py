"""
LeJEPA SOXX Market Stress POC-3 — Latent Dimension Sweep

Runs the full POC-1 pipeline (train → calibrate → score → evaluate) for three
latent dimension variants: 8, 16, and 32. All variants use seed=42.

Results are written to outputs/poc3/latent_dim_results.csv.
A one-line summary is appended to outputs/poc3/poc3_summary.md.
No files outside outputs/poc3/ are written.

Usage:
    python run_poc3_latent_sweep.py

Outputs:
    outputs/poc3/latent_dim_results.csv
...
def _print_console_summary(rows: list[dict]) -> None:
    print(f"\n=== Latent Dim Sweep Summary ===")
    print(f"{'dim':>5}  {'threshold':>10}  {'first_breach':>14}  {'verdict':>7}")
    print("-" * 44)
    for r in rows:
        breach = (
            r["lejepa_first_breach"].strftime("%Y-%m-%d")
            if r.get("lejepa_first_breach") is not None
            else "never"
        )
        verdict_str = "PASS" if r.get("all_pass") else "FAIL"
        thresh = r.get("lejepa_threshold")
        thresh_str = f"{thresh:.4f}" if thresh is not None and not (isinstance(thresh, float) and np.isnan(thresh)) else "nan"
        print(f"{r['latent_dim']:>5}  {thresh_str:>10}  {breach:>14}  {verdict_str:>7}")
...
def _append_poc3_summary(outdir: Path, rows: list[dict]) -> None:
    ...
    if lead_days:
        ...
        std_lead = float("nan")
    line = (
        f"latent_dim_sweep: {n_pass}/{n_total} dims pass, "
...
def run_latent_sweep(outdir: Path = Path("outputs/poc3")) -> pd.DataFrame:
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)

    config_base = Config()
    print("[latent_sweep] Building data pipeline...")
    ...
    for d in DIMS:
        print(f"\n[latent_sweep] Running latent_dim={d}...")
        _fix_seeds(42)
        config = Config(
            latent_dim=d,
            encoder_dims=(180, 64, d),
            predictor_dims=(d, 2 * d, d),
        )
        ...
        try:
            ...
            print(f"[WARNING] latent_dim={d}: CollapseError — {exc}")
            rows.append(
                {
                    "latent_dim": d,
        ...
        if latent_std_min < _COLLAPSE_THRESHOLD:
            print(
                f"[WARNING] latent_dim={d}: latent_std_min={latent_std_min:.4f} < "
        ...
        rows.append(_build_dim_row(d, config, verdict, latent_std_min))

    result_df = pd.DataFrame(rows)
    csv_path = outdir / "latent_dim_results.csv"
    result_df.to_csv(csv_path, index=False)
    print(f"\n[latent_sweep] Results written to {csv_path}")
...
def main() -> None:
    run_latent_sweep(outdir=Path("outputs/poc3"))
